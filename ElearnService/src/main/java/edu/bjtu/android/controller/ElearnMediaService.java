...
public class ElearnMediaService {
    ...
    public ResponseEntity<UrlResource> getImage(@PathVariable("name") String name, @RequestHeader HttpHeaders headers) throws Exception {
        ...
        String vpath = "file:///" + path + "\\" + contentPath + "\\" + name;
        UrlResource doc = new UrlResource(vpath);
        ResponseEntity<UrlResource> body = ResponseEntity.ok()
                .contentType(MediaTypeFactory.getMediaType(doc).orElse(MediaType.APPLICATION_OCTET_STREAM))
                .body(doc);
        return body;
    }
...
//	    }
    ...
    public String getMaterialMedia(@PathVariable("mid") String mid, @RequestHeader HttpHeaders headers) throws Exception {
        Material material = materialDao.selectByPrimaryKey(mid);

        String url = ip + ":" + port + "/" + contentPath + "/" + material.getMaterialUrl();
...
//        return getVideo(material.getMaterialUrl(), headers);
    ...
    public String getMaterialDoc(@PathVariable("mid") String mid, @RequestHeader HttpHeaders headers) throws Exception {
        Material material = materialDao.selectByPrimaryKey(mid);
...
//        }
    ...
    public String getMaterialVideoframe(@PathVariable("mid") String mid, @RequestHeader HttpHeaders headers) throws Exception {
        Material material = materialDao.selectByPrimaryKey(mid);
...
//        }
    ...
    public String getTeacherPhoto(@PathVariable("tid") String tid, @RequestHeader HttpHeaders headers) throws Exception {
        Teacher teacher = teacherDao.selectByPrimaryKey(tid);
        String url = ip + ":" + port + "/" + contentPath + "/" + teacher.getPhoto();
...
//        return getImage(teacher.getPhoto(), headers);
    ...
    public String getCourseAvatar(@PathVariable("cid") String cid, @RequestHeader HttpHeaders headers) throws Exception {
        Course course = courseDao.selectByPrimaryKey(cid);

        String url = ip + ":" + port + "/" + contentPath + "/" + course.getAvatar();
